...
describe('ProfileEditor component', () => {
  ...
  test('should mount without crashing', () => {
    ...
    expect(wrapper.find('.edit-profile-card .editor-header')
    .text()).toBe('Edit Profile');
  });

  test('should allow user change input values', () => {
    state.uploading = true;
    const wrapper = wrapperFn();
    wrapper.find('#edit-about').simulate('change',
    { target: { value: 'A changed person' } });
    expect(inputChangeSpy).toHaveBeenCalledTimes(1);
    wrapper.find('#edit-email').simulate('change',
    { target: { value: 'newsammy@test.com' } });
